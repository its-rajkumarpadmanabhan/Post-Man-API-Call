...
function App() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        // Fetch only the logs meant for React rendering
    ...
    fetchHistory();
    const interval = setInterval(fetchHistory, 1000); // Polling every 1 second
    return () => clearInterval(interval);
  }, []);

  // Inline styling for the table layout
  const styles = {
          ...
          {history.length === 0 ? (
            ...
            history.map((log) => (
              <tr key={log.id}>
                ...
                <td style={styles.td, {color: '#27ae60', fontWeight: 'bold'}}>{log.status}</td>
              </tr>
            ))
          )}
        </tbody>
